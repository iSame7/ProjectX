...
class MapCoordinator: BaseCoordinator<Void> {
    ...
    override public func start() -> Observable<Void> {
        ...
        showVenueDetials.subscribe { [weak self] (venue, venuePhotoURL) in
            ...
            guard let venueDetailsCoordinator: BaseCoordinator<Void> = self.venueDetailsModuleBuilder.buildModule(with: self.viewController, venue: venue, venuePhotoURL: venuePhotoURL)?.coordinator else {
